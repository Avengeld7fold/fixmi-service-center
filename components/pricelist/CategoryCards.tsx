      ...
      {categories.map((category, index) => {
        ...
        return (
          <button
            ...
            aria-pressed={active}
            style={{ scrollSnapAlign: "start", animationDelay: `${180 + index * 50}ms` }}
            className={`pricelist-enter group relative flex w-36 sm:w-40 md:w-44 aspect-square shrink-0 flex-col items-center justify-between rounded-[16px] lg:rounded-[20px] border bg-panel p-4 sm:p-5 lg:p-6 outline-none transition-[transform,border-color,box-shadow] duration-200 ease-out hover:-translate-y-1 active:scale-[0.97] lg:w-full lg:aspect-square focus-visible:border-primary ${
              active
