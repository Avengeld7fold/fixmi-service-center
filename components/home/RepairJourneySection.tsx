...
const BAYON_FONT = { fontFamily: "var(--font-bayon), sans-serif" } as const;
...
// ponytail: shared card inner layout — previously duplicated verbatim between desktop & mobile (~150 lines)
function StepCardBody({ step }: { step: JourneyStep }) {
  return (
    <>
      ...
      {/* Top 58%: Typography */}
      <div className="h-[58%] px-4 sm:px-5 xl:px-5 pt-4 sm:pt-5 xl:pt-4.5 pb-2 flex flex-col items-center justify-center text-center relative z-10 gap-1.5 -mt-1">
        <h3 className="font-bayon text-[1.45rem] sm:text-[1.75rem] uppercase text-[#f5f5f5] leading-[0.92] tracking-tight group-hover:text-primary transition-colors text-center" style={BAYON_FONT}>
          {step.title}
        </h3>
        <div className="font-bayon text-3xl sm:text-[2.65rem] text-primary leading-none text-center my-0.5" style={BAYON_FONT}>
          {step.stepNumber}
        </div>
      ...
      {/* Bottom 42%: Photo */}
      <div className="h-[42%] w-full relative bg-[#0d0d10] overflow-hidden rounded-b-xl lg:rounded-b-lg xl:rounded-b-xl flex items-center justify-center p-2 sm:p-2.5">
        {step.imageUrl ? (
          <div className="relative w-full h-full bg-white p-1 sm:p-1.5 rounded-[3px] shadow-[0_8px_20px_rgba(0,0,0,0.55),0_2px_6px_rgba(0,0,0,0.35)] flex items-center justify-center group-hover:scale-[1.03] transition-transform duration-300 ease-[cubic-bezier(0.23,1,0.32,1)]">
            <div className="relative w-full h-full overflow-hidden rounded-[2px] bg-[#1a1a1e]">
              <Image
                ...
                sizes="(max-width: 1024px) 340px, (max-width: 1200px) 280px, 320px"
              />
            </div>
          </div>
        ) : (
...
// ponytail: shared WhatsApp CTA button for desktop dock and mobile footer
function JourneyCta({ className = "" }: { className?: string }) {
  return (
    <a
      href={whatsappUrl()}
      target="_blank"
      ...
      className={`group inline-flex items-center justify-center gap-2 rounded-full bg-primary text-[#121212] font-bayon uppercase tracking-wider shadow-[0_8px_20px_rgba(0,0,0,0.85),0_2px_8px_rgba(255,107,0,0.35)] border border-black/20 transition-[transform,box-shadow,background-color] duration-200 ease-out hover:scale-105 active:scale-[0.97] whitespace-nowrap ${className}`}
      style={BAYON_FONT}
    >
...
export default function RepairJourneySection() {
  ...
  const repairSteps: JourneyStep[] = useMemo(
    () => [
      ...
      },
      {
        ...
        imageUrl: "/images/iphone-depth.webp",
        targetRotate: -3.0,
        targetY: 4,
        zIndex: 30,
      },
      {
        ...
        imageUrl: "/images/iphone-fixed.webp",
        targetRotate: 6.0,
        targetY: 36,
        zIndex: 40,
      },
        ...
        (context) => {
          ...
          if (reduceMotion) {
            repairSteps.forEach((step, idx) => {
              gsap.set(cards[idx], { y: `${step.targetY}px`, opacity: 1, scale: 1, rotate: step.targetRotate });
            });
            if (progressLineRef.current) progressLineRef.current.style.width = "100%";
            setActiveStep(4);
            return;
          }

          // Initial card entry positions
          const startRotates = [-8, 6, -6, 8];
          cards.forEach((card, i) => {
            gsap.set(card, { y: "80vh", opacity: 0, scale: 0.96, rotate: startRotates[i] });
          });
          ...
          const tl = gsap.timeline({
            scrollTrigger: {
              ...
              onUpdate: (self) => {
                ...
                setActiveStep(Math.min(4, Math.max(1, Math.floor(progress * 4) + 1)));
              },
            },
          });
          ...
          // ponytail: loop 4 slide-up phases instead of 4 copy-pasted blocks
          repairSteps.forEach((step, i) => {
            tl.to(
              cards[i],
              {
                y: `${step.targetY}px`,
  ...
  const getCardWidth = (container: HTMLDivElement) =>
    container.firstElementChild ? (container.firstElementChild as HTMLElement).offsetWidth + 16 : 300;

  const handleMobileScroll = () => {
    if (!mobileScrollRef.current) return;
    const container = mobileScrollRef.current;
    setMobileActiveStep(Math.min(3, Math.max(0, Math.round(container.scrollLeft / getCardWidth(container)))));
  };

  const scrollToMobileStep = (index: number) => {
    if (!mobileScrollRef.current) return;
    const container = mobileScrollRef.current;
    container.scrollTo({ left: index * getCardWidth(container), behavior: "smooth" });
  ...
  return (
    <div ref={containerRef} className="relative w-full bg-[#121212] text-white select-none">
      ...
      >
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center max-w-4xl w-full mx-auto px-4 z-0 pointer-events-none">
          <h2 className="font-bayon text-4xl xl:text-6xl uppercase text-[#f5f5f5] leading-[0.92] tracking-[-0.01em]" style={BAYON_FONT}>
            {dict.journey.heading}
          </h2>
        </div>

        {/* Fanned-out Cards Deck */}
        <div className="relative z-10 flex items-center justify-center w-full mx-auto px-4">
          <div className="relative inline-flex items-center">
            ...
            {/* Bottom-left Step Progress Dock */}
            <div className="absolute top-[calc(100%+5.5rem)] xl:top-[calc(100%+6.5rem)] 2xl:top-[calc(100%+8rem)] left-0 z-40 flex items-center pointer-events-auto">
              ...
              >
                ...
                <span className="font-bayon text-xl text-primary leading-none" style={BAYON_FONT}>
                  0{activeStep} / 04
                </span>
            ...
            {/* Desktop Anchored WhatsApp CTA */}
            <div
              className="absolute -bottom-5 xl:-bottom-6 -right-2 xl:-right-3 z-50 pointer-events-auto"
              style={{ transform: "translateY(36px) rotate(-6deg)", transformOrigin: "center center" }}
            >
              <JourneyCta className="px-4 xl:px-4.5 py-1.5 xl:py-2 text-xs xl:text-sm" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="lg:hidden relative w-full py-12 sm:py-16 px-4 overflow-hidden bg-[#121212]">
        <div className="text-center max-w-xl mx-auto mb-8 sm:mb-10 px-2">
          <h2 className="font-bayon text-3xl sm:text-4xl md:text-5xl uppercase text-[#f5f5f5] leading-[0.95] tracking-[-0.01em]" style={BAYON_FONT}>
            {dict.journey.heading}
          </h2>
            ...
            {repairSteps.map((step, idx) => (
              <button
                ...
                className={`px-3 sm:px-4 py-1 rounded-full font-mono text-[11px] sm:text-xs uppercase transition-[background-color,color,transform] duration-150 ease-out active:scale-[0.96] ${
                  mobileActiveStep === idx ? "bg-primary text-[#121212] font-bold shadow-md" : "text-neutral-400 hover:text-white"
                }`}
              >
                0{idx + 1}
              </button>
        ...
        {/* Mobile Step Indicators & Bottom CTA */}
            ...
            {repairSteps.map((_, idx) => (
              <button
                ...
                className={`transition-[width,background-color,transform] duration-200 ease-out active:scale-[0.95] rounded-full ${
                  mobileActiveStep === idx ? "w-7 h-1.5 bg-primary" : "w-2 h-1.5 bg-white/20 hover:bg-white/40"
                }`}
              />
            ))}
          </div>

          <JourneyCta className="px-5 py-2.5 text-xs sm:text-sm" />
        </div>
      </div>
    </div>
  );
