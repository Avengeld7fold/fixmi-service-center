...
import { Plus, ChevronDown, ChevronsUpDown, Download, Check } from "lucide-react";
...
export default function PricelistEditor({ categories }: { categories: Category[] }) {
  ...
  const addService = () => {
    ...
    setMessage(null);
    setAddError("");
    setAddSuccess(`Layanan "${name}" berhasil ditambahkan ke draf! Lengkapi harga model di bawah, lalu klik "Simpan Perubahan".`);
    ...
    // Auto-scroll ke kartu layanan yang baru ditambahkan
    setTimeout(() => {
      const el = document.getElementById(`admin-svc-${slug}`);
      if (el) {
  ...
  return (
    <>
      <div className="mt-8 space-y-6">
        ...
        {/* ── Top Save Banner (Saat ada perubahan draf) ── */}
        {dirty && (
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 rounded-2xl border border-primary/40 bg-gradient-to-r from-primary/15 via-[#1a1412] to-white/[0.02] p-3.5 sm:px-5 sm:py-3 shadow-[0_8px_30px_rgba(255,107,0,0.12)]">
            <div className="flex items-center gap-2.5">
              <span className="h-2 w-2 rounded-full bg-primary animate-pulse shrink-0" />
              <span className="text-xs sm:text-sm font-medium text-white">
                Ada perubahan draf yang belum disimpan ke server
              </span>
            </div>
            <div className="flex items-center justify-end gap-2 shrink-0">
              <button
                ...
                title="Unduh file pricelist.json editan ini langsung ke komputer"
                className="inline-flex items-center gap-1.5 rounded-xl border border-white/15 bg-white/[0.04] px-3 py-1.5 text-xs font-medium text-neutral-200 hover:bg-white/[0.08] hover:text-white transition-all active:scale-95"
              >
              ...
              <button
                ...
                onClick={discard}
                disabled={saving}
                className="rounded-xl border border-white/10 bg-white/[0.02] px-3 py-1.5 text-xs font-medium text-neutral-400 hover:text-white transition-all active:scale-95"
              >
                Batalkan
              </button>
              <button
                ...
                onClick={save}
                disabled={saving}
                className="rounded-xl bg-primary px-4 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-primary-light hover:brightness-105 transition-all active:scale-95"
              >
        ...
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4 sm:p-5 backdrop-blur-sm shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
          ...
          <div className="space-y-3">
            ...
            {addSuccess && (
              <p role="status" className="flex items-center gap-1.5 text-xs text-emerald-400 font-medium">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <div className="flex items-center justify-end gap-2.5 w-full sm:w-auto">
                <button
                  ...
                  onClick={downloadDraftJson}
                  title="Unduh file pricelist.json editan ini langsung ke komputer"
                  className="rounded-xl border border-white/[0.15] bg-white/[0.04] px-3.5 py-2 text-xs sm:text-sm font-medium text-neutral-200 transition-all hover:bg-white/[0.08] hover:text-white active:scale-95 text-center flex items-center gap-1.5"
                >
                  <Download className="h-3.5 w-3.5 text-primary" />
